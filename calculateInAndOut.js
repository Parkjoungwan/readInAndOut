    ...
    .on("end", () => {
      ...
      for (const name in userEntries) {
        userEntries[name].sort((a, b) => {
          let aTime;
          let bTime;
          if (a.inTime != undefined) aTime = a.inTime;
          else if (a.inTime == undefined) aTime = a.outTime;
          if (b.inTime != undefined) bTime = b.inTime;
          else if (b.inTime == undefined) bTime = b.outTime;
          return aTime - bTime;
        });
      }
      ...
      for (const name in userEntries) {
        ...
        for (let i = 0; i < entries.length; i += 2) {
          if (i + 1 < entries.length) {
            if (
              entries[i + 1].inTime != undefined ||
              entries[i].inTime == undefined
            ) {
              if (
                entries[i].inTime == undefined &&
                entries[i + 1].inTime == undefined
              )
                while (entries[i].inTime == undefined) i++;
              while (
                i + 1 < entries.length &&
                entries[i + 1].inTime != undefined
              )
            ...
            var outTime;
            if (i + 1 >= entries.length) outTime = inTime;
            else outTime = entries[i + 1].outTime;
            if (i + 2 < entries.length && entries[i + 2].outTime != undefined) {
              while (
                i + 2 < entries.length &&
                entries[i + 2].outTime != undefined
              )
                i++;
            }
            const duration = outTime
              .diff(inTime, ["hours", "minutes", "seconds"])
              .toObject();
            ...
            records.push({
              ...
              근무시간: `${String(Math.floor(duration.hours)).padStart(
                2,
                "0"
              )}:${String(Math.floor(duration.minutes)).padStart(
                2,
                "0"
              )}:${String(Math.floor(duration.seconds)).padStart(2, "0")}`,
            });
          }
        }
      }
